...
def dump(k, v):
    print('{}= {}'.format(k, binascii.hexlify(v)))
...
def server(seckey_b, my_node_id, message, keyBytes=72):
    ...
    dump('pubkey_X', pubkey_X.serialize())
    seckey_y = PrivateKey()
    dump('seckey_y', seckey_y.serialize())
    pubkey_Y = seckey_y.get_public()
    dump('pubkey_Y', pubkey_Y.serialize())
    pubkey_B = seckey_b.get_public()
    dump('pubkey_B', pubkey_B.serialize())
    dump('seckey_b', seckey_b.serialize())
    xy = seckey_y.get_shared_key(pubkey_X, hash_nil)
    dump('xy', xy)
    xb = seckey_b.get_shared_key(pubkey_X, hash_nil)
    dump('xb', xb)

    dump('fingerprint', my_node_id)
