...
func TestParseNetInfoCell(t *testing.T) {
	data := []byte{
		0x0, 0x0, 0x0, 0x0, 0x8, 0x59, 0xda, 0x79, 0x94, 0x4, 0x4, 0x7f, 0x0,
		0x0, 0x1, 0x1, 0x4, 0x4, 0x7f, 0x0, 0x0, 0x1, 0x0, 0x0, 0x0, 0x0, 0x0,
	...
	assert.Equal(t, int64(0x59da7994), ni.Timestamp.Unix())
	assert.Equal(t, "127.0.0.1", ni.ReceiverAddress.String())
	assert.Len(t, ni.SenderAddresses, 1)
	assert.Equal(t, "127.0.0.1", ni.SenderAddresses[0].String())
}
...
func TestParseNetInfoCellErrors(t *testing.T) {
	cases := [][]byte{
		{0, 1, 2},
